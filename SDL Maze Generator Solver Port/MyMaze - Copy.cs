...
namespace SDL_Maze_Generator_Solver_Port
{
	...
	enum MoveEnum { MOVE_LEFT, MOV_FORWARD, MOV_RIGHT, MOV_ERR };
	...
	class MyMaze
	{
		...
		private bool[,] _maze;
		Stack<Point> _solver;
		Stack<List<MoveEnum>> _solverChoiceStates;

		private Random _rand;

		public MyMaze(int width, int height, uint delay, bool isDemo)
		{
			// Initialize SDL
			...
			_window = SDL.SDL_CreateWindow("Maze Generator/Solver", SDL.SDL_WINDOWPOS_CENTERED, SDL.SDL_WINDOWPOS_CENTERED, _gridWidth * 10, _gridHeight * 10, SDL.SDL_WindowFlags.SDL_WINDOW_SHOWN);
			...
			_solver = new Stack<Point>();
			_solverChoiceStates = new Stack<List<MoveEnum>>();
		...
		private void _drawMaze()
		{
			...
			rect.x = (_gridWidth - 2) * 10;
			rect.y = (_gridHeight - 1) * 10;
			SDL.SDL_RenderFillRect(_renderer, ref rect);
		...
		private void _solve()
		{
			...
			_solver.Push(current);

			//ChoiceState choice = new ChoiceState();
			...
			// This means it's our first iteration, so move straight up
			if (flag)
			{
				next.X = current.X;
				next.Y = current.Y - 1;
				...
				//choice.previousChoice = 0;
				_solverChoiceStates.Push(availableChoices);
			...
			// Otherwise, try to move left, up, or right, in that order
			else
			{
				...
				{
					List<MoveEnum> activeState;// = new ChoiceState();
					while (true)
					{
						// There are no legal moves here, so pop back
						SDL.SDL_SetRenderDrawColor(_renderer, 0x00, 0x80, 0xff, 0xff);
						rect.x = current.X * 10;
						rect.y = current.Y * 10;
						SDL.SDL_RenderFillRect(_renderer, ref rect);
						...
						previous = _solver.Peek();
						_solver.Push(current);
						activeState = _solverChoiceStates.Pop();

						if (activeState.Count > 0) break;
					}
					availableChoices = activeState;
					//foreach (MoveEnum m in activeState)
					//{
					//    availableChoices.Add(m);
					//}
				}

				foreach (MoveEnum m in availableChoices)
				{
					if(_calculateDir(m, current, previous, ref next)) break;
				}
		...
		{
			_solver.Clear();
			Point position = new Point(_gridWidth - 2, _gridHeight - 1);
			_solver.Push(position);
		...
		private void _generateMaze(Point current, Point previous)
		{
			...
			for (int i = 0; i < numValidNeighbors; i++)
			{
				...
				if (_demoMode)
				{
					rect.x = ((next.X - current.X) / 2 + current.X) * 10;
					rect.y = ((next.Y - current.Y) / 2 + current.Y) * 10;
					SDL.SDL_RenderFillRect(_renderer, ref rect);
					SDL.SDL_RenderPresent(_renderer);
					SDL.SDL_Delay(_delay);
					rect.x = next.X * 10;
		...
		private void _playerMove(DirEnum direction)
		{
			...
			if (_prevPlayerPos.X == -1 && _prevPlayerPos.Y == -1 && direction == DirEnum.DIR_UP)
			{
				...
				rect.x = _curPlayerPos.X * 10;
				rect.y = _curPlayerPos.Y * 10;
				SDL.SDL_RenderFillRect(_renderer, ref rect);
